...
산출물은 ① 일별 성과 요약 ② 전략별 진단(옥석) ②-b **상태(국면·흐름·시장폭) 파악 정확도 점검(Phase 2.5)** ③ 시스템 보완 조치 ④ 전역 수익률 개선 ⑤ **전략별 보정방안(수집 데이터 기반 진입·청산·손절·사이징 튜닝)** ⑥ **데이터 공백 & 추가 지표 제안** ⑦ **신규 전략 발굴(수익 pocket → 가설, Phase 5)** + **멀티데이 발굴(선정×청산 교차, Phase 5-b)** 7부.
...
| **멀티데이(2-3주)** | `multiday-exit-comparison?fullPathsOnly=true` (일봉경로 보유D+N/트레일/MA/손절 시뮬 — **완주 코호트 필수**), `multiday-marks` (수집현황). ⚠️ 지정 전략만 수집되고 **대조군은 0행** — Phase 5-b 참조 |
...
## Phase 5-b. 멀티데이 발굴 (2026-09-03 추가 — 인트라데이 발굴과 데이터·논리가 다르다)

Phase 5는 전부 **인트라데이 horizon**(exit/close)으로 본다. 그런데 2026-09-03에 **같은 선정에 청산만 바꾸니
클러스터 보정 후 부호가 갈리는** 사례가 나왔다(C: 단순보유 D+1 −0.13%, 무장트레일(+5%→−2%) **+0.25%**, 완주 155건).
즉 **"이 선정은 졌다"는 판정이 청산 하나로 뒤집힐 수 있다** — 그래서 LIVE에서 제외된 전략(B·C·E·F·H)도
멀티데이 청산으로 다시 볼 가치가 있다. 인트라데이 발굴만 하면 이 축이 통째로 안 보인다.
...
(8/21 실측: D가 혼합에선 D+1 +0.57 → D+10 **+3.16**으로 단조 증가해 "오래 들수록 좋다"로 보였는데,
완주 표본만 고정하면 D+1 −1.27 → D+10 **−8.12**로 전부 음수이고 방향도 반대였다).

### 반사실을 무엇으로 대체하나 (대조군이 없으므로)
1. **같은 표본 안의 다른 청산 규칙** — 가장 강한 비교다. 같은 진입에 청산만 바꾸므로 선정 효과가 상쇄된다.
   ⚠️ 반드시 **단순보유(D+1/D+5/D+15)를 기준선**으로 함께 낼 것. 트레일이 단순보유를 못 이기면 규칙이 값을 못 한 것이다.
2. **유니버스 단순보유**(백테스트 쪽 `multiday-backtest`의 `universeHold`) — 선정 자체의 기여를 재는 축.
3. ⚠️ **인트라데이 대조군을 그대로 빌려오지 말 것** — 대조군은 종가까지만 추적되므로 D+5 비교의 반사실이 못 된다.

### 발굴 필터(멀티데이판)
1. **완주 코호트**(`fullPathsOnly=true`)로 볼 것.
2. **클러스터 가드** — `netExTopDayPct`로 부호가 유지되는가. ⚠️ 이게 멀티데이에서 특히 치명적이다:
   C는 완주 155건 중 **86.5%가 20260626 하루**라 모든 방식이 `clustered=true`이고, 그래서 `recommended`가
   `"표본부족·클러스터"`로 나온다. **그 라벨이 뜨면 어떤 방식도 채택 금지**다.
3. **단순보유 대비 우위** — 트레일/MA이탈/손절이 같은 표본의 단순보유를 이겨야 한다.
4. **거래일 분산** — `distinctDays ≥ 5`.
5. ⚠️ **손절 유무를 반드시 함께 볼 것** — 시뮬에 손절이 없는데 라이브엔 적응형 손절(−3~10%)이 걸린다.
   2026-09-03 실측: 같은 표본에 −7%를 적용하면 6/26 제외 +0.25→**+0.92**로 좋아지는데 ±20% 절사 후로는
   +2.07→**−0.96**으로 나빠졌다(히트 17.4%) — **두 관점이 반대라 안 갈린다**. 어느 쪽도 근거로 단정하지 말 것.

### 산출 형식
**[선정 전략] × [청산 규칙] · 완주 n · 거래일 · net · netExTopDay · 단순보유 대비 → 가설 → 검증 계획**

⚠️ 채택은 **섀도우 신규 전략**으로 넘긴다(2026-09-03 전략 P가 그 템플릿 — 진입은 기존 전략과 **완전히 동일하게** 두고
청산만 바꿔 변수를 하나로 고정했다. 조건을 복제하지 말고 **같은 프로퍼티를 읽게** 해야 나중에 갈라지지 않는다).
⚠️ 멀티데이 전략을 만들면 **게이트 채점 horizon도 함께**(`multiday`) 맞출 것 — 분 마크로 채점하면 게이트가
검증한 적 없는 청산으로 실주문을 열어준다.
