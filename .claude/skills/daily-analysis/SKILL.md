...
| 시장 폭 | `market-breadth` |
| **멀티데이 성과(핵심)** | `multiday-exit-comparison?fullPathsOnly=true&horizonDays=15`(**두 파라미터 다 필수** — 아래 함정 참조), `multiday-marks`(전략별 수집현황) |
...
LIVE 화이트리스트(**2026-09-15 실측 10개: A·C·D·F·G·H·I·J·L·P** — B는 9/8 제외, F·H는 9/12 재편입.
이 목록은 추측하지 말고 매번 `sudo docker exec sa-app printenv | grep TRADING_LIVE_STRATEGIES`로
재확인할 것 — 스킬 문서의 목록은 반드시 낡는다)의 **실제 경제성**을 판정하는 1차 도구. 인트라데이 net이 아니라 여기 수치가 "이
전략이 돈을 버는가"의 답이다.

```
multiday-exit-comparison?fullPathsOnly=true&horizonDays=15   # ⚠️ 두 파라미터 다 필수 — 아래 참조
multiday-marks                                   # 수집 현황 + ohlcCoveragePct(고·저가 커버리지 — 낮으면 손절이 과소 반영된다)
```

🔴 **`horizonDays`를 빼면 13전략 전부 `fullPaths=0`이 나온다**(2026-09-15 실측으로 재확인). 보유 상한이
60거래일로 늘어난 뒤로 `fullPathsOnly=true`의 "완주" 정의가 **D+60 도달**이 돼버려, 아직 60거래일을 넘긴
표본이 하나도 없기 때문이다. 응답은 에러가 아니라 전 전략 `recommended: "표본부족·클러스터"`로 조용히
나오므로 **그대로 읽으면 "판정 불가"로 오독하게 된다.** `horizonDays=15`처럼 지평을 명시하면 경로를 그
거래일에서 잘라 모든 표본을 같은 창에서 해소시킨다. L·P처럼 도입이 늦어 마크가 15일에 못 미치는 전략은
`horizonDays=5`(또는 `multiday-marks`의 `maxMarkDays` 이하)로 한 번 더 돌려야 표본이 잡힌다.

⚠️ **`fullPathsOnly` 없이 부르는 혼합 모드도 쓰지 말 것** — 미결(트리거 미발동) 경로를 표본에서 빼는데
실측상 미결분이 지고 있는 쪽이라 남는 수치가 낙관으로 편향된다.

**읽는 법**:
- 🔴 **`라이브규칙(…)` 줄이 "지금 실제로 하는 매매"의 net이다**(2026-09-10 신설. 손절%는 설정에서 읽어
  제목에 박히므로 고정값으로 기억하지 말 것 — 2026-09-15 현재 `트레일 -2.0%·손절 -12.0%`).
  나머지 줄(보유 D+N·트레일·MA이탈·손절)은 전부 **손절이 없는 가상 세계**의 수치라, 그것만 보면 라이브가
  실제로 확정하는 손실이 net에서 빠진다. 전략 판정은 이 줄로 하고, 다른 줄은 "바꾼다면 얼마나 나아지나"의
  비교 대상으로만 읽을 것. `recommended`가 `라이브규칙…`이면 **현행 유지가 답**이라는 뜻이다(동률이면
  현행을 고르도록 tie-break가 걸려 있다 — 검증 안 된 청산으로 갈아타지 않기 위해).
- ⚠️ **`multiday-marks`의 `ohlcCoveragePct`를 먼저 볼 것** — 손절은 종가가 아니라 **장중 저가**에서 발사되므로,
  고·저가가 없는 마크는 종가 판정으로 degrade한다(= 손절 히트 과소 = net 낙관). 커버리지가 낮으면
  `POST /admin/backfill-daily-mark-ohlc`(`daily_price`에서 소급, 재실행 안전)를 돌린 뒤 다시 읽는다.
...
🔴 **보유기간 단축은 제안하지 말 것 — 이 시스템은 "존버 시스템"으로 확정됐다(2026-09-15 사용자 결정).**
`recommended`가 `보유 D+1`처럼 짧은 보유로 나와도, D+N 열의 net·초과수익이 라이브규칙보다 높아도,
**D+1 전환·스윙(`TRADING_SWING_STRATEGIES`) 편입·멀티데이 청산 목록에서 제외를 제안하지 않는다.**
경위: 2026-09-15 분석이 G(초과 +1.54%p, n=318·22거래일·비클러스터·LOO +0.62) · H(+0.19%p, LOO +1.34)의
D+1 전환을 근거와 함께 올렸고, 사용자가 **명시적으로 거부하며 "앞으로도 그만 얘기해"라고 못 박았다.**
즉 보유기간은 이제 데이터로 정하는 파라미터가 아니라 **사용자가 정한 시스템 정체성**이다 —
데이터가 우위를 보이는 것과 그것을 채택 근거로 쓰는 것은 별개다.
→ **D+N 열은 "형태 진단"용으로만 읽는다**(예: "이 전략의 엣지가 어느 구간에 있나", "D+10 양수가
드리프트인가 알파인가"). 청산 관련 조치를 낼 거면 **보유를 유지한 채** 가능한 것으로 한정할 것 —
손절폭(`TRADING_ADAPTIVE_STOP_*`)·트레일 폭(`TRADING_MULTIDAY_EXIT_DROP_PCT`)·진입 필터(`SIGNAL_*`)·
화이트리스트 제외. (2026-09-10 미결이던 "G·H의 D+1 전환 판단 대기"는 이 결정으로 **기각 종결**됐다.)
