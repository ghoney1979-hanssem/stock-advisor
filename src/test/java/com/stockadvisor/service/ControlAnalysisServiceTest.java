...
class ControlAnalysisServiceTest {
    ...
    private List<TradeOutcome> outcomes(String strategy, int n, double closePct, String reason) {
        ...
        for (int i = 0; i < n; i++) {
            TradeOutcome o = new TradeOutcome(strategy, null, "0059" + i, "20260630", buy);
    ...
    void diagnose_비스윙_exit기준_손실_거른게더나으면_MISCALIBRATED() {
        ...
        for (int i = 0; i < 15; i++) {   // 진입(손실)
            TradeOutcome o = new TradeOutcome("BREAKOUT_E", null, "e" + i, "20260630", 10_000);
        ...
        for (int i = 0; i < 20; i++) {   // 거른 게 더 나음
            TradeOutcome o = new TradeOutcome("BREAKOUT_E", null, "r" + i, "20260630", 10_000);
    ...
    void diagnose_스윙은_nextClose로_판정() {
        ...
        List<TradeOutcome> rows = new ArrayList<>();
        for (int i = 0; i < 15; i++) {
            TradeOutcome o = new TradeOutcome("MEAN_REVERSION_C", null, "00c" + i, "20260630", 10_000);
