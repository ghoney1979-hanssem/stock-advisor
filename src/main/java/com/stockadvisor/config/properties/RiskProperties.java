 ...
 *                              <b>0이면 비활성(기존 동작)</b>. 2026-08-14 실측 근거: <b>LIVE 15건이 전부 09:01~09:24(27분)에 진입</b>,
 *                              09:07 시점 노출 5,012,064원(순자산의 47% — 한도 5,370,338원 도달)으로 6분 만에 리스크 예산이 소진됐고,
 *                              그 구간이 갭업 고점이라 −188,665원(계좌 −1.78%)을 냈다(K 4건이 같은 되돌림에 손절선 동반 청산).
 *                              리스크 예산이 "선착순"으로 배분돼 개장 갭 구간에 <b>상관도 높은 롱이 집중</b>되는 구조를 시간 분산으로 완화한다.
 *                              ⚠️ 초기 서술 정정: "예산 소진이 오후까지 이어져 회복 구간에 미참여"는 <b>틀렸다</b> —
 *                              노출은 현재 보유 기준이라 청산되면 회복된다(09:20 여유 발생 → 10:42엔 0원). 실제로 물린 구간은
 *                              09:07~09:20·09:24~09:38 두 번뿐이고, 오후 미진입은 K의 발동 창(09:00~09:30) 설계와
 *                              D-KOSPI perf-gate 차단 때문이다. 따라서 이 knob의 근거는 "갭 구간 상관 노출 축소" 하나다.
